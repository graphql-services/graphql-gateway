...
const getRemoteSchema = (url) => __awaiter(this, void 0, void 0, function* () {
    ...
    let link = apollo_link_context_1.setContext((request, previousContext) => {
    ...
    link = apollo_link_error_1.onError(({ response }) => {
        if (typeof response === 'undefined') {
            return;
        }
        response.errors =
            response.errors &&
                response.errors.map(err => new graphql_1.GraphQLError(err.message, undefined, null, err.locations && err.locations.map(loc => loc.line), err.path, null, err.extensions));
    }).concat(link);
